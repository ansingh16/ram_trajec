...
def animate(i):
    ...
    if (i == 0):
        ...
        vy1[i, 0] = 0.0#v_0.value  # v_0.value  # -28.7996765414#

        #vx2[i, 0] = 0.0  # -0.135847174988#
        #vy2[i, 0] = 0.0  # 0.0451986511525#

        ax1[i, 0] = -(G.value * M_ICM.value / (dist(x1[i, 0], x2[i, 0], y1[i, 0], y2[i, 0]) ** 2)) * ((x1[i, 0]) / (dist(x1[i, 0], x2[i, 0], y1[i, 0], y2[i, 0])))

        ay1[i, 0] = -(G.value * M_ICM.value / (dist(x1[i, 0], x2[i, 0], y1[i, 0], y2[i, 0]) ** 2)) * ((y1[i, 0]) / (dist(x1[i, 0], x2[i, 0], y1[i, 0], y2[i, 0])))
    ...
    if (0 < i <= nstep - 1):
        print x1[i-1,0],vx1[i-1,0],ax1[i-1,0],x2[i-1,0]
        ...
        ax1[i, 0] = (G.value * M_r.value / (dist(x1[i, 0], x2[i, 0], y1[i, 0], y2[i, 0]) ** 2)) * ((x2[i, 0] - x1[i, 0]) / (dist(x1[i, 0], x2[i, 0], y1[i, 0], y2[i, 0])))

        ay1[i, 0] = (G.value * M_r.value / (dist(x1[i, 0], x2[i, 0], y1[i, 0], y2[i, 0]) ** 2)) * ((y2[i, 0] - y1[i, 0]) / (dist(x1[i, 0], x2[i, 0], y1[i, 0], y2[i, 0])))

        #ax2[i, 0] = (G.value * M_gal.value / (dist(x1[i, 0], x2[i, 0], y1[i, 0], y2[i, 0]) ** 2)) * ((x1[i, 0] - x2[i, 0]) / (dist(x1[i, 0], x2[i, 0], y1[i, 0], y2[i, 0])))

        #ay2[i, 0] = (G.value * M_gal.value / (dist(x1[i, 0], x2[i, 0], y1[i, 0], y2[i, 0]) ** 2)) * ((y1[i, 0] - y2[i, 0]) / (dist(x1[i, 0], x2[i, 0], y1[i, 0], y2[i, 0])))
...
scat, = ax.plot([], [], linestyle='', ms=2, marker='o', color='b', label=r'$\theta = \pi/6, V_{in} = 1 \times V_{200}$')
...
def init():
    ...
    circle = plt.Circle((0.0, 0.0), 2 * R_vir.value, color='blue', alpha=0.1)
    ax.add_artist(circle)
    ax.legend(loc=2)
    plt.annotate(s='', xy=(2 * R_200.value, 0), xytext=(0, 0), arrowprops=dict(arrowstyle='<->'))
    ax.text(R_vir.value / 2, -1.0, r'$R=2R_{vir}$', fontsize=10)
    time_text.set_text('')
...
anim.save('theta20_vel1.0.mp4', writer=writer)

...
tau = np.sqrt((4.0 * (np.pi ** 2) * a ** 3) / (G * (gal._md_tot + IGM._M_200))).to('Myr').value

dt = tau*u.Myr / para['n1']
...
for i in range(1,n-1):
    ...
    axgal = (G * IGM._M_200 / (dyn.dist(xgal, xclus, ygal, yclus) ** 2))*(( - xgal) / (dyn.dist(xgal, xclus, ygal, yclus)))

    aygal = (G * IGM._M_200 / (dyn.dist(xgal, xclus, ygal, yclus) ** 2))*(( - ygal) / (dyn.dist(xgal, xclus, ygal, yclus)))

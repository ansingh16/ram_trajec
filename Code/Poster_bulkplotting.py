...
def plot_styling(bx,flag):
    ...
    if(flag=='rstrip'):
            ...
            bx.set_ylabel(r"$R_{\mathrm{strip}}/R_{\mathrm{dg}}$",  fontsize=16)
            bx.set_xlabel(r"$R_{\mathrm{c}}/R_{\mathrm{vir}}$",  fontsize=16)
            bx.minorticks_on()
            bx.tick_params(axis='both', which='minor', length=3, width=2, labelsize=14)
            bx.tick_params(axis='both', which='major', length=5, width=2, labelsize=14)

    if (flag == 'M_removed'):

            bx.set_ylabel(r"$M_{\mathrm{removed}}$",  fontsize=16)
            bx.set_xlabel(r"$R_{\mathrm{c}}/R_{\mathrm{vir}}$",  fontsize=16)
            bx.minorticks_on()
            bx.tick_params(axis='both', which='minor', length=3, width=2, labelsize=14)
            bx.tick_params(axis='both', which='major', length=5, width=2, labelsize=14)
...
def differentM(list1,En):
        ...
        ax1.set_ylabel(r"$\mathbf{R_{\mathrm{strip}}/R_{\mathrm{dg}}}$",  fontsize=16)
        ax1.set_xlabel(r"$\mathbf{R_{\mathrm{c}}/R_{\mathrm{vir}}}$",  fontsize=16)

        ax1.minorticks_on()
        ax1.tick_params(axis='both', which='minor', length=5, width=2, labelsize=14)
        ax1.tick_params(axis='both', which='major', length=8, width=2, labelsize=14)

        fig2, ax2 = plt.subplots(figsize=(8, 6))
        ax2.set_ylabel(r"$\mathbf{M_{\mathrm{removed}}}$",  fontsize=16)
        ax2.set_xlabel(r"$\mathbf{R_{\mathrm{c}}/R_{\mathrm{vir}}}$",  fontsize=16)

        ax2.minorticks_on()
        ax2.tick_params(axis='both', which='minor', length=5, width=2, labelsize=14)
        ax2.tick_params(axis='both', which='major', length=8, width=2, labelsize=14)
...
if __name__ == '__main__':
        ...
        if (type == 'c'):
            ...
            for z in ['z00', 'z05', 'z10']:
                ...
                '''
                M = ['$M_{gal} = 10^{10} M_{\odot}$', '$M_{gal} = 10^{11} M_{\odot}$', '$M_{gal} = 10^{12} M_{\odot}$', \
                    '$M_{gal} = 10^{13} M_{\odot}$']

                for Mass in ['M10e10','M10e11','M10e12','M10e13']:
                    ...
                    ax[i].set_title(titl[i],  fontsize=16)
                    bx[i].set_title(titl[i],  fontsize=16)

                    ax[i].set_ylabel(r"$R_{\mathrm{strip}}/R_{\mathrm{dg}}$", fontsize=16)
                    ax[i].set_xlabel(r"$R_{\mathrm{c}}/R_{\mathrm{vir}}$", fontsize=16)
                    #ax[i].set_title('z = ' + redshift)
                    ax[i].minorticks_on()
                    ax[i].tick_params(axis='both', which='minor', length=3, width=2, labelsize=14)
                    ax[i].tick_params(axis='both', which='major', length=5, width=2, labelsize=14)

                    bx[i].set_ylabel(r"$M_{\mathrm{removed}}$", fontsize=16)
                    bx[i].set_xlabel(r"$R_{\mathrm{c}}/R_{\mathrm{vir}}$", fontsize=16)
                    #bx[i].set_title('z = ' + redshift)
                    bx[i].minorticks_on()
                    bx[i].tick_params(axis='both', which='minor', length=3, width=2, labelsize=14)
                    bx[i].tick_params(axis='both', which='major', length=5, width=2, labelsize=14)


                    ax[i].set_xlim(-0.4, 2.1)
                    bx[i].set_xlim(-0.4, 2.1)
                    bx[i].set_ylim(0.0, 1.1)
        ...
        if (type == 'b'):
            ...
            theta1 = str(input('theta?'))
            theta = 'theta' + theta1

            lmb = str(input('lambda?'))
            lmbda = 'lambda' + lmb
            ...
            fig1, (ax1) = plt.subplots(1, 1, sharey=True, figsize=(8, 6))

            #fig1, (ax1) = plt.subplots(1, 1, sharey=True, figsize=(8, 6))
            #fig2, (bx1) = plt.subplots(1, 1, sharey=True, figsize=(8, 6))
            ...
            titl = ['z = 0.0' + r', $\theta$ = ' +r'$\pi$/6'+r', $\lambda$ = '+str(lmb)+r', $\delta$ = '+str(delt)+r', $f_{g}$ = '+str(f_g)]
            #for z in ['z00', 'z05', 'z10']:
            for z in ['z00']:

                j=0
                vel=['$V_{\mathrm{in}}/V_{\mathrm{vir}}$ = 0.0', '$V_{\mathrm{in}}/V_{\mathrm{vir}}$ = 0.5','$V_{\mathrm{in}}/V_{\mathrm{vir}}$ = 1.0']
        ...
        if(type=='a'):
            ...
            titl=['z = 0.0' + r', $V_{gal}/V_{vir}$ = ' +str(vel1)+r', $\lambda$ = '+str(lmb)+r', $\delta$ = '+str(delt)+r', $f_{g}$ = '+str(f_g)]
            ...
            for z in ['z00']:
                ...
                for th in ['theta00','theta150','theta300']:
                    ...
                    if (th == 'theta150'):
                        theta = th.replace('theta150', r'$\theta =$15.0 $^\circ$')

                    ax[j].plot(dR,dr,linewidth=2.5,label=theta.replace('00','0.0 $^\circ$ '))
                    bx[j].plot(dR,dM,linewidth=2.5,label=theta.replace('00','0.0 $^\circ$ '))

                    ax[j].set_ylabel(r"$R_{\mathrm{strip}}/R_{\mathrm{dg}}$", fontsize=16)
                    ax[j].set_xlabel(r"$R_{\mathrm{c}}/R_{\mathrm{vir}}$", fontsize=16)
                    ax[i].set_title(titl[i])
                    ax[j].minorticks_on()
                    ax[j].tick_params(axis='both', which='minor', length=3, width=2, labelsize=14)
                    ax[j].tick_params(axis='both', which='major', length=5, width=2, labelsize=14)

                    bx[j].set_ylabel(r"$M_{\mathrm{removed}}$", fontsize=16)
                    bx[j].set_xlabel(r"$R_{\mathrm{c}}/R_{\mathrm{vir}}$", fontsize=16)
                    bx[i].set_title(titl[i])
                    bx[j].minorticks_on()
                    bx[j].tick_params(axis='both', which='minor', length=3, width=2, labelsize=14)
                    bx[j].tick_params(axis='both', which='major', length=5, width=2, labelsize=14)

                    ax[j].set_xlim(-0.4, 2.1)
                    bx[j].set_xlim(-0.4, 2.1)
                    bx[j].set_ylim(0.0, 1.1)
            ...
            fig1.savefig(En +Mass+vel +'ThetaVariation_R_vs_r_strip.png', dpi=600)
            fig2.savefig(En + Mass+vel+'ThetaVariation_Mvariation_R_vs_M_removed.png', dpi=600)

            fig1.savefig(En +Mass+vel +'ThetaVariation_R_vs_r_strip.eps', format='eps', dpi=600)
            fig2.savefig(En +Mass+ vel+'ThetaVariation_Mvariation_R_vs_M_removed.eps', format='eps', dpi=600)
        ...
        if(type=='9'):
            ...
            for redshift in ['0.0','0.5','1.0']:
                ...
                for delt in ['0.1','0.3']:
                    ...
                    ax[k].set_ylabel(r"$\mathbf{R_{\mathrm{strip}}/R_{\mathrm{dg}}}$",  fontsize=16)
                    ax[k].set_xlabel(r"$\mathbf{R_{\mathrm{c}}/R_{\mathrm{vir}}}$",  fontsize=16)
                    ax[k].set_title('z = ' + redshift, fontweight='bold')
                    ax[k].minorticks_on()
                    ax[k].tick_params(axis='both', which='minor', length=3, width=2, labelsize=14)
                    ax[k].tick_params(axis='both', which='major', length=5, width=2, labelsize=14)

                    bx[k].plot(data[:, 0], M_removed, line[i],color=colors[i], linewidth=1.5, label=r'$\delta$ = ' +delt)
                    bx[k].set_ylabel(r"$\mathbf{M_{\mathrm{removed}}}$",  fontsize=16)
                    bx[k].set_xlabel(r"$\mathbf{R_{\mathrm{c}}/R_{\mathrm{vir}}}$",  fontsize=16)
                    bx[k].set_title('z = ' + redshift, fontweight='bold')
                    bx[k].minorticks_on()
                    bx[k].tick_params(axis='both', which='minor', length=3, width=2, labelsize=14)
                    bx[k].tick_params(axis='both', which='major', length=5, width=2, labelsize=14)
        ...
        if(type=='8'):
            ...
            '''
                         ...
                         ax[k].set_xlim(5, 14)
                         ax[k].set_ylabel(r"$\mathbf{R_{\mathrm{strip}}/R_{\mathrm{dg}}}$",  fontsize=16)
                         ax[k].set_xlabel(r"$\mathbf{log (Mass(M_{\odot}))}$",  fontsize=16)
                         #ax[k].set_title('z = '+redshift,fontweight= 'bold')
                         ax[k].minorticks_on()
                         ax[k].tick_params(axis='both', which='minor', length=3, width=2, labelsize=14)
                         ax[k].tick_params(axis='both', which='major', length=10, width=2, labelsize=14)
        ...
        if(type=='7'):
            ...
            for redshift in ['0.0','0.5','1.0']:
                ...
                ax[k].semilogy(data[:, 4], data[:, 3], col[i] + 'o', linewidth=2, ms=10.0,label=r"$r_{\mathrm{strip}}$")
                # ax1.set_ylim(-0.05, )
                ax[k].set_xlim(0.5, 3.5)
                ax[k].set_ylabel(r"$\mathbf{log r(kpc)}$",  fontsize=16)
                ax[k].set_xlabel(r"$\mathbf{v_{f}}$",  fontsize=16)

                ax[k].set_title('z = ' + redshift, fontweight='bold')

                ax[k].minorticks_on()
                ax[k].tick_params(axis='both', which='minor', length=5, width=2, labelsize=14)
                ax[k].tick_params(axis='both', which='major', length=10, width=2, labelsize=14)
